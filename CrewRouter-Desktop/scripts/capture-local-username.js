...
app.whenReady().then(async () => {
  ...
  if (phase === 'first') {
    ...
    await waitFor(win, "location.hostname === '127.0.0.1' && (location.pathname === '/console' || location.pathname === '/console/')", 60000);
    const state = await waitFor(win, "document.body.innerText.includes('模型库') && !document.body.innerText.includes('登录') && !document.body.innerText.includes('正在加载')", 30000) && await win.webContents.executeJavaScript("({ href: location.href, text: document.body.innerText, unavailable: document.getElementById('modelLibraryContent')?.innerText.includes('Personal 版本地服务不提供共享模型库。'), noBinding: document.getElementById('modelLibraryBindingSummary')?.innerText.includes('不需要绑定当前 Key'), keySelectorHidden: getComputedStyle(document.getElementById('modelLibraryKeySelector'))?.display === 'none' })", true);
    if (!state.href.includes('127.0.0.1') || !state.text.includes('模型库') || !state.unavailable || !state.noBinding || !state.keySelectorHidden) throw new Error(`Automatic local start failed: ${JSON.stringify(state)}`);
