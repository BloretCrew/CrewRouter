...
- 总结弹窗记录独立的 `_summaryModalSessionKey`，弹窗复制与重新生成均使用弹窗目标会话；关闭详情时清理该状态。
- 分页仅在成功响应后更新完成态和触发自动加载；失败保留可重试按钮，不再误报“已到最早消息”。滚动自动加载也要求存在游标。
- 总结弹窗采用单一阶段文字：初始为“正在阅读会话...”，收到首个 SSE delta 后切换为“正在生成...”。
- 压缩会话详情移除 `LIMIT 2000`，以完整记录集计算 `total`、`hasMore` 与 `nextCursor`；`server/store/passport.js` 未修改。

## 已验证项

- `node --check public/js/app.js`：通过。
- `node --check server/routes/sessions-view.js`：通过。
- `node --check server/utils/usage-compress.js`：通过。
- `npm run build`：通过（release 构建完成）。
- `git diff --check`：通过。
