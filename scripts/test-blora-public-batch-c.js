...
assert.match(playgroundJs, /if \(streamFailed\) throw/);
assert.match(playgroundJs, /streamErrorMessage/);
assert.match(playgroundJs, /this\.escapeHtml\(r\.finishReason\)/);
assert.match(playgroundJs, /this\.escapeHtml\(r\.model\)/);
...
assert.match(playgroundJs, /const previousMessages = this\.messages\.slice\(\)/);
assert.match(playgroundJs, /rollbackRequest\(\)/);
assert.match(playgroundJs, /saveMessagesToConv\(convId\)/);
assert.match(playgroundJs, /pg-retry-btn/);
...
assert.match(serverJs, /streamAbortController\?\.signal\.aborted/);
assert.match(serverJs, /lastStatus = 504/);
assert.match(serverJs, /if \(streamTimeout\) \{ clearTimeout\(streamTimeout\)/);
assert.match(serverJs, /streamAbortController = isStream \? new AbortController\(\) : null/);
