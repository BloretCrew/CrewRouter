...
const pages = files.map((name) => { const file = path.join(pagesDir, name); const html = fs.readFileSync(file, 'utf8'); const loaded = scriptsAndInit(html); const dom = scan(html); const legacyModalContainers = (html.match(/class=["'][^"']*\b(?:modal|[^"']*modal[^"']*)\b[^"']*["']/gi) || []).length; const legacyDialogScript = loaded.scripts.filter((s) => /\/js\/dialog\.js/.test(s)).length; const legacyDialogApiCalls = (html.match(/\b(?:showModal|closeModal|openDialog|closeDialog)\s*\(/g) || []).length; return { page: rel(file), blora: { css: html.includes('/blora/blora.css'), darkTokens: html.includes('/blora/tokens.dark.css'), auto: html.includes('/blora/auto.js'), foundation: html.includes('/js/blora-foundation.js'), scope: /<body\b[^>]*class=["'][^"']*\bblora-page\b/.test(html), version: [...html.matchAll(/\/blora\/(?:blora\.css|tokens\.dark\.css|auto\.js)\?v=([^"']+)/g)].map((m) => m[1]) }, dom: { ...dom, legacyModalContainers, nativeDialogs: dom.dialogs, legacyDialogScript, legacyDialogApiCalls, staticOnly: true, dynamicTemplates: 'conservative-unobserved', runtime: 'not-observed' }, initialization: loaded.initialization, scripts: loaded.scripts }; });
