...
class ConsoleApp {
  ...
  async loadDesktopSettingsEmbed() {
    ...
    try {
      ...
      target.querySelector('#desktopAutoConnect').onchange = (event) => preferenceChanged('确定修改 Desktop 启动自动连接设置吗？', event.target);
      target.querySelector('#desktopNotifications').onchange = (event) => preferenceChanged('确定修改桌面通知设置吗？', event.target);
      target.querySelector('#desktopUpdateChecks').onchange = (event) => preferenceChanged('确定修改更新检查设置吗？', event.target);
      target.querySelector('#desktopTheme').onchange = (event) => { const next = event.target.value; reloadDesktop(); confirmAction('确定切换 Desktop 主题吗？', async () => { target.querySelector('#desktopTheme').value = next; await savePreferences(); }); };
