...
class ConsoleApp {
  ...
  async loadDesktopSettingsEmbed() {
    ...
    try {
      ...
      target.querySelector('#desktopRestartLocal')?.addEventListener('click', async () => { confirmAction('确定重启本地 Server 吗？当前本地连接会短暂中断。', async () => { await bridge.restartLocal?.(); reloadDesktop(); }); });
      target.querySelector('#desktopStopLocal')?.addEventListener('click', async () => { confirmAction('确定停止本地 Server 吗？停止后本地 CrewRouter 将不可用。', async () => { await bridge.stopLocal?.(); reloadDesktop(); }); });
      ...
      target.querySelector('#desktopRestartApp')?.addEventListener('click', () => { confirmAction('确定重启 CrewRouter Desktop 吗？', () => bridge.restartApp?.()); });
      target.querySelector('#desktopQuitApp')?.addEventListener('click', () => { confirmAction('确定关闭 CrewRouter Desktop 吗？', () => bridge.quit?.()); });
