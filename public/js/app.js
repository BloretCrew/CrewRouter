...
class ConsoleApp {
  ...
  async loadDesktopSettingsEmbed() {
    ...
    try {
      ...
      target.querySelectorAll('[data-profile-switch]').forEach((button) => button.addEventListener('click', async () => { const profile = profiles.find((item) => item.id === button.dataset.profileSwitch); if (!window.confirm(`确定切换到“${profile?.name || '这个 CrewRouter'}”吗？`)) return; try { await bridge.switchProfile(button.dataset.profileSwitch); } catch (error) { target.querySelector('#desktopSettingsMessage').textContent = error.message; } }));
      ...
      target.querySelectorAll('[data-profile-delete]').forEach((button) => button.addEventListener('click', async () => { if (!window.confirm('确定删除这个实例吗？删除后将从 Desktop 的已保存实例中移除。')) return; try { await bridge.deleteProfile(button.dataset.profileDelete); reloadDesktop(); } catch (error) { target.querySelector('#desktopSettingsMessage').textContent = error.message; } }));
      target.querySelector('#desktopAutoConnect').onchange = () => { if (!window.confirm('确定修改 Desktop 启动自动连接设置吗？')) { target.querySelector('#desktopAutoConnect').checked = !target.querySelector('#desktopAutoConnect').checked; return; } return bridge.saveDesktopSettings({ autoConnect: target.querySelector('#desktopAutoConnect').checked, notifications: target.querySelector('#desktopNotifications').checked, updateChecks: target.querySelector('#desktopUpdateChecks').checked, theme: target.querySelector('#desktopTheme').value }).then(reloadDesktop); };
      target.querySelector('#desktopNotifications').onchange = () => { if (!window.confirm('确定修改桌面通知设置吗？')) { target.querySelector('#desktopNotifications').checked = !target.querySelector('#desktopNotifications').checked; return; } return bridge.saveDesktopSettings({ autoConnect: target.querySelector('#desktopAutoConnect').checked, notifications: target.querySelector('#desktopNotifications').checked, updateChecks: target.querySelector('#desktopUpdateChecks').checked, theme: target.querySelector('#desktopTheme').value }).then(reloadDesktop); };
      target.querySelector('#desktopUpdateChecks').onchange = () => { if (!window.confirm('确定修改更新检查设置吗？')) { target.querySelector('#desktopUpdateChecks').checked = !target.querySelector('#desktopUpdateChecks').checked; return; } return bridge.saveDesktopSettings({ autoConnect: target.querySelector('#desktopAutoConnect').checked, notifications: target.querySelector('#desktopNotifications').checked, updateChecks: target.querySelector('#desktopUpdateChecks').checked, theme: target.querySelector('#desktopTheme').value }).then(reloadDesktop); };
      target.querySelector('#desktopTheme').onchange = () => { if (!window.confirm('确定切换 Desktop 主题吗？')) { reloadDesktop(); return; } return bridge.saveDesktopSettings({ autoConnect: target.querySelector('#desktopAutoConnect').checked, notifications: target.querySelector('#desktopNotifications').checked, updateChecks: target.querySelector('#desktopUpdateChecks').checked, theme: target.querySelector('#desktopTheme').value }).then(reloadDesktop); };
      target.querySelector('#desktopRestartLocal')?.addEventListener('click', async () => { if (!window.confirm('确定重启本地 Server 吗？当前本地连接会短暂中断。')) return; try { await bridge.restartLocal?.(); reloadDesktop(); } catch (error) { target.querySelector('#desktopSettingsMessage').textContent = error.message; } });
      target.querySelector('#desktopStopLocal')?.addEventListener('click', async () => { if (!window.confirm('确定停止本地 Server 吗？停止后本地 CrewRouter 将不可用。')) return; try { await bridge.stopLocal?.(); reloadDesktop(); } catch (error) { target.querySelector('#desktopSettingsMessage').textContent = error.message; } });
      target.querySelector('#desktopCopyDiagnostics').addEventListener('click', async () => { try { await navigator.clipboard.writeText(JSON.stringify(await bridge.getDiagnostics?.(), null, 2)); target.querySelector('#desktopSettingsMessage').textContent = '已复制'; } catch (error) { target.querySelector('#desktopSettingsMessage').textContent = error.message; } });
      target.querySelector('#desktopRestartApp')?.addEventListener('click', () => { if (window.confirm('确定重启 CrewRouter Desktop 吗？')) bridge.restartApp?.(); });
      target.querySelector('#desktopQuitApp')?.addEventListener('click', () => { if (window.confirm('确定关闭 CrewRouter Desktop 吗？')) bridge.quit?.(); });
