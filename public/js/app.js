...
class ConsoleApp {
  ...
  async loadDesktopSettingsEmbed() {
    ...
    try {
      ...
      target.querySelectorAll('[data-profile-switch]').forEach((button) => button.addEventListener('click', async () => { try { await bridge.switchProfile(button.dataset.profileSwitch); } catch (error) { target.querySelector('#desktopSettingsMessage').textContent = error.message; } }));
      target.querySelectorAll('[data-profile-rename]').forEach((button) => button.addEventListener('click', async () => { const name = window.prompt('重命名', profiles.find((item) => item.id === button.dataset.profileRename)?.name || ''); if (!name) return; try { await bridge.renameProfile(button.dataset.profileRename, name); reloadDesktop(); } catch (error) { target.querySelector('#desktopSettingsMessage').textContent = error.message; } }));
      target.querySelectorAll('[data-profile-delete]').forEach((button) => button.addEventListener('click', async () => { if (!window.confirm('确定删除这个实例吗？')) return; try { await bridge.deleteProfile(button.dataset.profileDelete); reloadDesktop(); } catch (error) { target.querySelector('#desktopSettingsMessage').textContent = error.message; } }));
      target.querySelector('#desktopAutoConnect').onchange = () => bridge.saveDesktopSettings({ autoConnect: target.querySelector('#desktopAutoConnect').checked, notifications: target.querySelector('#desktopNotifications').checked, updateChecks: target.querySelector('#desktopUpdateChecks').checked, theme: target.querySelector('#desktopTheme').value }).then(reloadDesktop);
      target.querySelector('#desktopNotifications').onchange = target.querySelector('#desktopAutoConnect').onchange;
      target.querySelector('#desktopUpdateChecks').onchange = target.querySelector('#desktopAutoConnect').onchange;
      target.querySelector('#desktopTheme').onchange = target.querySelector('#desktopAutoConnect').onchange;
      target.querySelector('#desktopRestartLocal')?.addEventListener('click', async () => { try { await bridge.restartLocal?.(); reloadDesktop(); } catch (error) { target.querySelector('#desktopSettingsMessage').textContent = error.message; } });
      target.querySelector('#desktopStopLocal')?.addEventListener('click', async () => { try { await bridge.stopLocal?.(); reloadDesktop(); } catch (error) { target.querySelector('#desktopSettingsMessage').textContent = error.message; } });
