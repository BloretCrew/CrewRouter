...
async function report(payload) { payload = { ...payload, event_id: eventId(payload.event_id) }; const { allows } = require('./filter'); if (!allows(payload.event, payload.tool_name)) { writeLog({ profile: loadConfig()?.current_profile, event: payload.event, ok: true, status: null, error: 'filtered' }); return true; } let token = null; try { token = await getAccessToken(); } catch (err) { enqueue(payload, err.message); } const cfg = loadConfig(); const url = cfg && String(cfg.url || '').replace(/\/$/, ''); if (!url || !token) { enqueue(payload, 'missing configuration or credential'); return false; } return postEvent(url, token, payload); }
