...
const { hookPath, install, uninstall } = require('./hooks');
...
function inspect(name) { const c = CLIENTS[name]; if (!c) throw new Error(`未知客户端: ${name}`); const file = c.path(); let exists = false; try { exists = fs.existsSync(file); } catch {} return { name, supported: c.supported, config_path: file, exists, helper_installed: name === 'grok' && exists, conflict: false, note: c.supported ? '可安全管理 CrewRouter Hook' : '仅探测，不自动修改客户端配置' }; }
...
module.exports = { CLIENTS, inspect, list, change };
